...
GenerateResult* generate_full(
    ...
    // 确保 prompt_len 不超过 max_len
    int actual_prompt_len = prompt_len;
    if (actual_prompt_len > max_len) {
        actual_prompt_len = max_len;
    }

    int* tokens = (int*)malloc(max_len * sizeof(int));
    memcpy(tokens, prompt_ids, actual_prompt_len * sizeof(int));
    free(prompt_ids);

    int current_len = actual_prompt_len;
    ...
    result->prompt_len = actual_prompt_len;
    result->generated_len = current_len - actual_prompt_len;
...
GenerateResult* generate_full_with_kv_cache(
    GPTModel* model,
    Tokenizer* tokenizer,
    const char* prompt,
    GenerateConfig* config
    ...
    // 创建 logits 张量
    int logits_shape[] = {vocab_size};
    Tensor* last_logits = tensor_zeros(1, logits_shape);

    // Prefill 阶段: 处理整个 prompt
    int prefill_logits_shape[] = {actual_prompt_len, vocab_size};
    Tensor* prefill_logits = tensor_zeros(2, prefill_logits_shape);

    model_forward_prefill(model, tokens, actual_prompt_len, kv_cache, NULL, prefill_logits);

    // 获取最后一个位置的 logits
    model_get_last_logits(prefill_logits, actual_prompt_len, last_logits);
    tensor_free(prefill_logits);
    ...
    for (int i = 0; i < config->max_new_tokens; i++) {
        ...
        // Decode: 只处理新 token
        model_forward_decode(model, next_token, current_len - 1, kv_cache, NULL, last_logits);
    }
...
char* generate_with_kv_cache(
    GPTModel* model,
    Tokenizer* tokenizer,
    const char* prompt,
    GenerateConfig* config
) {
    GenerateResult* result = generate_full_with_kv_cache(model, tokenizer, prompt, config);
    if (result == NULL) return NULL;
...
int generate_stream_with_kv_cache(
    GPTModel* model,
    Tokenizer* tokenizer,
    const char* prompt,
    GenerateConfig* config,
